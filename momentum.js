...
(() => {
  ...
  window.addEventListener('wheel', event => {
    if (reduced.matches || event.ctrlKey || Math.abs(event.deltaX) > Math.abs(event.deltaY) ||
        document.body.matches('.is-loading, .index-open') || event.target.closest('dialog, input, textarea, select')) return;
    ...
    target = Math.max(0, Math.min(target + delta * 1.8, document.documentElement.scrollHeight - innerHeight));
